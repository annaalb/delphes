  ...
  for (size_t i = 0; i < 2; i++) { // CHS or PUPPI
    for (size_t k = 0; k < 5; k++) { // eta bins
      ...
      for (size_t l = 0; l < 3; l++) {
        hist_all[l][i][k] = ((TH1F*)f[l]->Get(name[i]+"_RecoJet_pt_all_"+eta[k]));
        hist_matched[l][i][k] = ((TH1F*)f[l]->Get(name[i]+"_RecoJet_pt_matched_"+eta[k]));
    ...
    mg->GetXaxis()->SetTitle("RecoJet p_{T}");
    ...
    legend->SetHeader(name[i]+", p_{T, gen}>10 GeV, p_{T, reco}>20 GeV, "+eta_names[k]);
    ...
    canvas->SaveAs(outdir  + name[i]+eta[k]+"_purity.pdf");
  //  canvas->SaveAs(outdir  + name[i]+eta[k]+ "_purity.root");
    }// end eta bins
  } // end jet loop

...
void comparison_plots_PU_genjets_VBF()
{
  //SetStyle();
  SetupGlobalStyle();
  ...
  //TString variable[9] = {"_jet_time", "_jet_time_minus_PV", "_jet_pt_weighted_time", "_jet_pt_weighted_time_minus_PV", "_jet_R_weighted_time", "_jet_R_weighted_time_minus_PV", "_track_DZ", "_track_T", "_track_TOuter"};
  TString variable[2] = {"_track_DZ", "_track_T"};
  ...
  for (size_t i = 0; i < 3; i++) { // CHS or PUPPI, genjet or PUgenjet // change here
    ...
    track_PU[i][k] = ((TH1F*)f->Get(category[2]+"_"+name[i]+IsPU[k]+variable[l]+eta[e]));
  ...
  //legend->Draw();
  // save output
  TString outdir = "PLOTS/10kevents/comparison_plots/signal_track_rejection/"; // change here

...
void comparison_plots_VBF_QCD()
{
  ...
  TFile *f = new TFile("PLOTS/10kevents/macro_jets_snowmass_signalTrackRejection/results.root");
  //TFile *f_QCD = new TFile("PLOTS/10kevents/QCD/macro_jets_snowmass/results.root");
  TFile *f_QCD = new TFile("PLOTS/10kevents/QCD/macro_QCD_jets/results.root");
  ...
  for (size_t i = 0; i < 2; i++) { // CHS or PUPPI
    ...
    track_PU[i][k] = ((TH1F*)f_QCD->Get(name[i]+IsPU[k]+variable[l]+eta[e]));
  ...
  //legend->Draw();
  // save output
  TString outdir = "PLOTS/10kevents/comparison_VBF_QCD_no_matching/";

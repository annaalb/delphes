...
void comparison_plots_VBF_PU()
{
  //SetStyle();
  SetupGlobalStyle();
  // read from results.root
  TFile *f = new TFile("PLOTS/10kevents/macro_jets_snowmass_jet_time/results.root");
  TFile *f2 = new TFile("PLOTS/10kevents/macro_jets_snowmass_signalTrackRejection/results.root");
  ...
  TString variable[6] = {"_jet_time", "_jet_time_minus_PV", "_jet_arrivaltime", "_jet_arrivaltime_minus_PV", "_jet_pt_weighted_time", "_jet_pt_weighted_time_minus_PV"};
  ...
  for (size_t i = 0; i < 2; i++) { // CHS or PUPPI
    //for (size_t k = 0; k < 3; k++) {
      for (size_t l = 0; l < 6; l++) {  // variables
        for (size_t e = 0; e < 4; e++) { // eta bins
  //  track_VBF[i][k] = ((TH1F*)f->Get(category[0]+"_matched_"+name[i]+IsPU[k]+"_track_DZ"));
    //track_PU[i][k] = ((TH1F*)f->Get(category[3]+"_matched_"+name[i]+IsPU[k]+"_track_DZ"));
    track_VBF[i][k] = ((TH1F*)f->Get(category[0]+"_"+name[i]+variable[l]+eta[e]));
    track_b[i][k] = ((TH1F*)f->Get(category[1]+"_"+name[i]+variable[l]+eta[e]));
    track_PU[i][k] = ((TH1F*)f->Get(category[2]+"_"+name[i]+variable[l]+eta[e]));
  ...
  //legend->Draw();
  // save output
  TString outdir = "PLOTS/10kevents/comparison_plots/signal_track_rejection/";
  canvas->SaveAs(outdir  + name[i]+variable[l]+eta[e]+".pdf");
  canvas->SaveAs(outdir  + name[i]+variable[l]+eta[e]+ ".root");
} // end eta bins
  }
//  }
}
